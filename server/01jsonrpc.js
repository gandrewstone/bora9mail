...
jsonRpc = function(opts, method, params, callback, errback, path) {
  var time = Date.now();
  var requestJSON;
  ...
  var requestOptions = {
    ...
    agent: false,
    rejectUnauthorized: opts.ssl && opts.sslStrict !== false
  };
  
  if (opts.ssl && opts.sslCa) {
    requestOptions.ca = opts.sslCa;
  }
    
  // use HTTP auth if user and password set
  if (opts.user && opts.pass) {
    requestOptions.auth = opts.user + ':' + opts.pass;
  }
