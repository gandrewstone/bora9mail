...
UserDeviceSyncingData = function()
  {
  this.labels = { "Inbox": {count:0}, "Sent": { count:0}, "Drafts": {count:0} };
  }
...
storeLocalUserData = function(userdata, username, password)
  {
  if (username==null) username = Session.get("username");
  if (password==null) password = Session.get("password");  // Grab the user's password from the session if available
