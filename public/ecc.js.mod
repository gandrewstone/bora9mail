...
ecc.decrypt = function(deckey, ciphertext) {
  ...
  if(!key)
    key = kem.$keys[obj.tag] = kem.unkem(sjcl.codec.base64.toBits(obj.tag));
...
function eccAPI(algoName) {
  ...
  return {
    ...
    importPublic: function(keyStr) {
      ...
      return new algo.publicKey(key.curve, sjcl.codec.base64.toBits(key.hex));
    },
...
function exportPublic(keyObj) {
  var obj = keyObj.get();
  return keyObj.$curve +
         sjcl.codec.base64.fromBits(obj.x) +
         sjcl.codec.base64.fromBits(obj.y);
}
function exportSecret(keyObj) {
  return keyObj.$curve + sjcl.codec.base64.fromBits(keyObj.get());
}
